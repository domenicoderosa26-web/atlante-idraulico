...
Le API ufficiali sono documentate da Normattiva in <https://dati.normattiva.it/assets/come_fare_per/API_Normattiva_OpenData.pdf>. Ambiente di produzione: `https://api.normattiva.it/t/normattiva.api`. `POST /bff-opendata/v1/api/v1/ricerca/aggiornati` segnala gli atti aggiornati; `POST /bff-opendata/v1/api/v1/atto/dettaglio-atto-urn` verifica identità e metadati. La prima API non fornisce la vigenza complessiva affidabile dei singoli articoli. Per questo lo stato resta **DA VERIFICARE** finché un evento di modifica non viene rilevato; **MODIFICATA** indica un evento nell'elenco, non la validità di ogni articolo. Abrogazioni e sostituzioni richiedono un riscontro specifico: il codice dell'atto modificante è registrato, senza dedurne automaticamente la natura. La data del controllo sulla scheda è il riscontro del dettaglio o dell'ultimo evento, non la data del checkpoint tecnico giornaliero.
